...
* Thread는 고유의 thread id, PC(program counter), register set, stack을 가진다. 또한 한 process에 속한 여러 thread는 code section, data section, 기타 resource(open files and signal 등)을 공유한다.

### 응용 프로그램의 예

1. Web browser

Web browser는 이미지 또는 텍스트를 표시하는 하나의 thread와 network로부터 데이터를 가져오는 또 다른 thread를 가질 수 있다. 
...
* Balance(균형) : parallel processing 하는 task가 의미가 있도록 균형이 잡혀야 된다. 각자 균등한 기여도를 가져야 한다는 말이다.
...
Multiple core에서 어떤 동일 연산을 수행할 때, data 양이 많아서 data를 나눠서 실행하도록 분배하는 개념이다.
...
## 4.2 다중 쓰레드 모델(Multithreading Models)

* User threads : kernel 위에서 지원되며 kernel의 지원 없이 관리된다. thread programming이 가능한 경우를 말한다.
* Kernel threads : OS에 의해 직접 지원되고 관리된다. Multi-process로 돌아가는 형태를 말하며 거의 모든 현대의 OS가 Kernel threads를 지원한다.
...
## 4.3 쓰레드 라이브러리(Threads Library)

Library란 자주 쓰이는 기능을 모아놓은 것으로 programmer가 thread를 생성하고 운영할 수 있도록 해주는 것이 threads library이다.

### 구현하는 2가지 방법

* Kernel support 없이 user space 상에 두는 libary
...
Signal이란 UNIX에서 처음 사용한 용어로 process의 어떤 event 발생을 알리는 것을 말하며 이에 대한 처리는 signal handler가 정해진 procedure를 통해 해결한다. Signal은 다음 2가지 형태로 전달될 수 있다.

* Synchronous : 예측 가능하며, 내부적 event에 의한 것이다. 예를 들어 *illegal memory access(포인터 문제)* 나 *division by zero(0으로 나누기)* 문제가 있다. 참고로, 몫 연산자인 /을 사용할 때는 방금 언급한 문제와 더불어서 원하는 참값이 아닐 수도 있다는 생각을 하고 있어야 한다.
* Asynchronous : 외부 event에 의한 것이며, 예로 Ctrl+c를 이용해서 콘솔을 종료시키는 *terminating a process* 가 있다. 
...
Thread는 해당 process의 data section을 공유하지만 상황에 따라, 어떤 data에 대하여 자신만의 copy를 유지할 필요가 있는데 그 때 사용하는 것이 바로 TLS이다.